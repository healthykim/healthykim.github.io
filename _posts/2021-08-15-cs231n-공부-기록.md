...
음 어느 방향이 loss 함수의 값이 줄어드는 방향이냐고 물어보면 문돌이인 나도 미적분은 배웠기 때문에 미분을 떠올릴 수 있다. 그라디언트가 뭔가 했는데 그냥 각 차원으로 편미분한 것들의 벡터을 의미하는 것이었다. 영어로는 다음과 같이 표현된다.
...
진짜 내용이 왤케 없어? 코드를 빨리 붙일까봐. 근데 그건 날잡고 하고 싶어서 아직 노우 노우
